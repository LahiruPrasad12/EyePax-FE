<template>
  <section>
    <b-modal
      v-model="is_create_stock_modal_active"
              ...
              <validation-observer ref="createItemValidation">
                <b-row>
                  <b-col md="6">
                    <validation-provider #default="{ errors }" name="qty" rules="required">
                      <b-field :message="errors[0]" :type="errors[0] ? 'is-danger' : 'is-success'"
                               label="Item*">
                        <v-select :options="item" v-model="form.item_code" style="color: rgb(180, 180, 180);" placeholder="Select an Item">
                        </v-select>
                      </b-field>
                    </validation-provider>
                  </b-col>
                  ...
                  <b-col class="mt-5" md="12">
                    <b-button :disabled="is_btn_loading" :loading="is_btn_loading" class="ml-5" style="float: right" type="is-info" @click="createStock">Add</b-button>
